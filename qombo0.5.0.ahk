...
LoadWindow() {
	rawwin := ""
	FileRead, rawwin, %A_WorkingDir%\qombosettings.json
	win := JSON.parse(rawwin)
	WinMove, qombo v*, , win.X, win.Y , win.W, win.H
	return
}
...
SaveWindow() {
	...
	win.W := winW
	win.H := winH
	winpos := JSON.stringify(win)
	FileDelete, %A_WorkingDir%\qombosettings.json
	FileAppend, %winpos%, %A_WorkingDir%\qombosettings.json
	return
}
...
ParseAccount() {
	...
	mygroupinfo := GetAPICall("http://localhost:12391/groups/member/", myaddress)
...
ParseTrades(url, trades) {
    ...
    if (trades == "forsale") {
        for k, v in JSON.parse(rawtrades)
        {
            ...
            LV_Add("", v.qortAmount, v.expectedBitcoin, cointype, v.expectedBitcoin/v.qortAmount, HowLongAgo(v.creationTimestamp), v.creationTimestamp, sellername, v.qortalCreator)
        }
        LV_ModifyCol()
    }
    else if (trades == "sold") {
        for k, v in JSON.parse(rawtrades)
        {
            LV_Add("", v.qortAmount, v.btcAmount, "", v.btcAmount/v.qortAmount, HowLongAgo(v.tradeTimestamp), v.tradeTimestamp, "", "")
        }
        LV_ModifyCol()
        LV_ModifyCol(3, 0)
        LV_ModifyCol(7, 0)
